...
export class ParticleSheetBase {
    ...
    constructor(opts: BaseOpts) {
        ...
        let firstIndex = parseInt(
            localStorage.getItem(`${this.baseKey}-view`) || '0',
        )
    ...
    setViewDataIndex = (index: number) => {
        ...
        this.viewDataCurrent = JSON.parse(
            JSON.stringify(this.viewData[this.viewDataIndex]),
        )
    ...
    makeViewChooser = () => {
        ...
        this.viewData.forEach((data, i) => {
            let viewTitleEl = createElement(
                'span',
                { class: 'views__title' },
                data.key || `view ${i}`,
            )
            let viewNumEl = createElement(
                'span',
                { class: 'views__num' },
                `${i + 1}`,
            )
            let viewEl = createElement(
                'button',
                { class: 'views__btn', 'data-index': `${i}` },
                [viewNumEl, viewTitleEl],
            )
    ...
    setGeometryStuff = () => {
        if (!this.particles)
            return console.warn('no particles, cant set geometry stuff')
        let view = this.viewData[this.viewDataIndex]

        view.sheets.forEach((sheet, i) => {
            let params = sheet.params
            if (params.position)
                this.particles[i].position.copy(params.position)
            if (params.rotation)
                this.particles[i].rotation.copy(params.rotation)
            if (params.blending?.default)
                this.particles[i].material.blending = params.blending.default
        })
    }
    ...
    setUniforms = () => {
        let view = this.viewData[this.viewDataIndex]
        if (!this.particles)
            return console.warn('no particles, cant set uniforms')
        ...
        view.sheets.forEach((sheet, i) => {
            ...
            let folderTitle =
                this.particles.length > 1 ? `uniforms-${i}` : 'uniforms'
    ...
    setParticlesGui = () => {
        ...
        view.sheets.forEach((sheet, i) => {
            if (!this.particles[i]) return
            let title =
                this.viewDataCurrent.sheets.length > 1
                    ? `params-${i}`
                    : 'params'
    ...
    getParams = () => {
        ...
        view.sheets.forEach((sheet, i) => {
            ...
            let storedParams = JSON.parse(
                localStorage.getItem(`${this.currentKey}-${title}`)!,
            )
            if (storedParams && setVals) {
                if (storedParams.sheetWidth)
                    params.sheetWidth = storedParams.sheetWidth
                if (storedParams.sheetHeight)
                    params.sheetHeight = storedParams.sheetHeight
                ...
                if (params.position && storedParams.position)
                    params.position = new THREE.Vector3().copy(
                        storedParams.position,
                    )
                if (params.rotation && storedParams.rotation)
                    params.rotation = new THREE.Euler().copy(
                        storedParams.rotation,
                    )
    ...
    update = (time: number) => {
        ...
        if (this.cameraListening && this.glsCameraControls) {
            let vals = this.glsCameraControls.gui.save() as {
                controllers: { [key: string]: any }
            }
            let cameraCurrent = localStorage.getItem(
                `${this.currentKey}-camera`,
            )
