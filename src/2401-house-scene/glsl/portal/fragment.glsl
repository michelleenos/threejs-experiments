...
//     _st *= _grid;
//     vec2 pos = rotate2d(noise(_st - uTime * _time) * PI) * _st;
...
void main() {
    ...
    float a1 = cnoise(vec3(uv.x * -10.0, uv.y * -8.0, uTime * 0.2)) * PI;
    uv *= rotate2d(a1);

    float angle = atan(uv.y, uv.x);
    float offset = d0 + (angle/ (PI * 2.0)) * uSpiralTightness;
    // float offset = (log(d0) / log(e * 5.0) + (angle/ (2.0 * PI)) * 0.5);
    float circles = mod(offset - uTime * uSpeed, uSpiralTightness);
    float c = circles;
    c *= 1.0 / uSpiralTightness;
